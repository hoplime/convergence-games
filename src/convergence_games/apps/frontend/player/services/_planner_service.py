...
class PlannerService:
    ...
    async def get_planner_data(  # noqa: C901
        self, *, event: Event, user: User, time_slot_id: int | None
    ) -> PlannerData:
        ...
        games_and_preferences = (await self._session.execute(games_and_preferences_this_time_slot_stmt)).all()
        scheduled_time_slots_stmt = select(Session.game_id, Session.time_slot_id).where(Session.committed)
        scheduled_time_slots = (await self._session.execute(scheduled_time_slots_stmt)).all()
        ...
        all_party_members_game_playeds = (
            (await self._session.execute(all_party_members_game_playeds_stmt)).scalars().all()
        )
        user_game_playeds: dict[int, UserGamePlayed] = {
...
async def provide_planner_service(transaction: AsyncSession) -> PlannerService:
    return PlannerService(transaction)
